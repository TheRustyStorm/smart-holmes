...
fn main() {
    ...
    let dependencies = generate_dependencies(&dependency_config, &smart_home.devices);
    println!("{} dependencies", dependencies.len());
    
    println!("{} working dependencies", smart_home.amount_fullfilled_dependencies(&dependencies));

    let mut smart_home_all = smart_home.clone();
    smart_home_all.update_all();
    println!("Updating all");
    println!("{} working dependencies", smart_home_all.amount_fullfilled_dependencies(&dependencies));

    let mut smart_home_random = smart_home.clone();
    smart_home_random.update_random();
    println!("Updating random");
    println!("{} working dependencies", smart_home_random.amount_fullfilled_dependencies(&dependencies));

    let mut smart_home_smart = smart_home.clone();
    smart_home_smart.update_smart();
    println!("Updating Smart");
    println!("{} working dependencies", smart_home_smart.amount_fullfilled_dependencies(&dependencies));
